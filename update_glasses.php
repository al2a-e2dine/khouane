...
if (isset($_POST['submit'])) {
  ...
  $get_id=$_POST['get_id'];
      ...
      $q="UPDATE `glasses` SET `designation`='$des',`ref`='$ref',`marque`='$marque',`sexe`='$sexe',`enf_or_adu`='$enf_or_adu',`fournisseur_id`='$fournisseur_id',`qte`='$qte',`seuil_min`='$seuil_min',`prix_a`='$prix_a',`prix_v`='$prix_v' WHERE `id`='$get_id'";
      //echo $q;exit();
      $r=mysqli_query($dbc,$q);
          ...
          <div class="col-lg-7">
            <div class="p-5">
              <div class="text-center">
                <h1 class="h4 mb-4" style="color: #5f1411">Modifier lunettes</h1>
                <?php
                if (isset($msg)) {
                ?>
              ...
              ?>
              <form class="user" action="update_glasses.php?id=<?= $get_id ?>" method="post">
                <div class="form-group">
                  <!-- <label>Designation</label> -->
                    <input type="text" class="form-control form-control-user" value="<?= $row['designation'] ?>" name="des" required>
                </div>
                 <div class="form-group">
                  <!-- <label>Référence</label> -->
                    <input type="text" class="form-control form-control-user" value="<?= $row['ref'] ?>" name="ref" required>
                </div>
                <div class="form-group">
                  <!-- <label>Marque</label> -->
                    <input type="text" class="form-control form-control-user" value="<?= $row['marque'] ?>" name="marque" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <label for="sel1">Mineur ou majeur</label>
                  <select class="form-control" id="sel1" name="enf_or_adu" required>
                    <option value="<?= $row['enf_or_adu'] ?>"><?= $row['enf_or_adu'] ?></option>
                    <option value="Mineur">Mineur</option>
                    <option value="Mjaeur">Mjaeur</option>
                  </select>
                </div>
                <div class="form-group">
