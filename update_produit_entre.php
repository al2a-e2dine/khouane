...
if (isset($_POST['submit'])) {
  ...
  $get_id=$_POST['get_id'];
      ...
      $q="UPDATE `produit_entre` SET `article`='$article',`marque`='$marque',`fournisseur_id`='$fournisseur_id',`qte`='$qte',`seuil_min`='$seuil_min',`prix_a`='$prix_a',`prix_v`='$prix_v' WHERE `id`='$get_id'";
      //echo $q;exit();
      $r=mysqli_query($dbc,$q);
...
<head>
  ...
  <title>Modifier produits d'entretien</title>
          ...
          <div class="col-lg-7">
            <div class="p-5">
              <div class="text-center">
                <h1 class="h4 mb-4" style="color: #5f1411">Modifier produits d'entretien</h1>
                <?php
                if (isset($msg)) {
                ?>
              ...
              ?>
              <form class="user" action="update_produit_entre.php?id=<?= $get_id ?>" method="post">
                <div class="form-group">
                  <label for="sel1">Article</label>
                  <select class="form-control" name="article" required>
                    <option value="<?= $row['article'] ?>"><?= $row['article'] ?></option>
